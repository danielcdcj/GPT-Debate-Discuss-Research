...
export function Sidebar() {
  ...
  const mobileSidebarOpen = useDebateStore((s) => s.mobileSidebarOpen);
  ...
  if (sidebarCollapsed) {
    return (
      <div className="hidden lg:flex w-12 bg-slate-900 border-r border-slate-800 flex-col items-center py-4 gap-2 shrink-0">
        <button
  ...
  const sidebarContent = (
    <div className="w-[280px] lg:w-[260px] bg-slate-900 border-r border-slate-800 flex flex-col h-full">
      <div className="p-4 border-b border-slate-800 flex items-center justify-between">
        ...
        <button
          ...
          className="text-slate-400 hover:text-white p-1 rounded hover:bg-slate-800 transition-colors hidden lg:block"
        >
      ...
      <div className="p-3">
        <Button onClick={() => setShowNewRoom(true)} className="w-full" size="sm">
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="flex-1 overflow-y-auto px-3 pb-3 space-y-1">
        {rooms.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-xs text-slate-600 mt-1">Create a room to get started</p>
          </div>
        ) : (
          rooms
  ...
  return (
    <>
      ...
      <NewRoomDialog isOpen={showNewRoom} onClose={() => setShowNewRoom(false)} />
    </>
  );
}
